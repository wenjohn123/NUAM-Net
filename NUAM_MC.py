...
from utils import  ResBlock, ConvBlock, Up, Compute_z, Channel_Attention_Block, EnhanceLayers
...
class Decoder(nn.Module):
    def __init__(self, device):
        ...
        #补充LAB色域
        #self.po1_encoder = Encoder(6)


        self.pr_conv = ResBlock(64)
        self.po_conv = ResBlock(64)
        self.po1_conv = ResBlock(64)
        ...
        # self.po1_Up3 = Up()
        # self.po1_UpConv3 = ConvBlock(ch_in=128, ch_out=64)
        # self.po1_Up2 = Up()
        # self.po1_UpConv2 = ConvBlock(ch_in=128, ch_out=64)

        out_conv = []
        out_conv.append(ResBlock(64*2))
        out_conv.append(ResBlock(64*2))
        out_conv.append(nn.Conv2d(64*2, 3, kernel_size=1, padding=0))
        ...
        self.attention = Channel_Attention_Block(64)
        self.enhance = EnhanceLayers()

    def forward(self, Input, InputLab, Target, training=True):

        pr_x1, pr_x2, pr_x3, pr_x4 = self.pr_encoder.forward(Input)

        pr1_x1, pr1_x2, pr1_x3, pr1_x4 = self.pr_encoder.forward(InputLab)

        if training:
            po_x1, po_x2, po_x3, po_x4 = self.po_encoder.forward(torch.cat((Input, Target), dim=1))
            # po1_x1, po1_x2, po1_x3, po1_x4 = self.po_encoder.forward(torch.cat((InputLab, TargetLab), dim=1))
            # x4->x3
            pr_x4 = self.pr_conv(pr_x4)
            pr1_x4 = self.pr_conv(pr1_x4)
            po_x4 = self.po_conv(po_x4)
            #po1_x4 = self.po_conv(po1_x4)
            pr_d3 = self.pr_Up3(pr_x4)
            pr1_d3 = self.pr_Up3(pr1_x4)
            po_d3 = self.po_Up3(po_x4)
            #po1_d3 = self.po_Up3(po1_x4)
            # x3->x2
            pr_d3 = torch.cat((pr_x3, pr_d3), dim=1)
            pr1_d3 = torch.cat((pr1_x3, pr1_d3), dim=1)
            po_d3 = torch.cat((po_x3, po_d3), dim=1)
            #po1_d3 = torch.cat((po1_x3, po1_d3), dim=1)
            pr_d3 = self.pr_UpConv3(pr_d3)
            pr1_d3 = self.pr_UpConv3(pr1_d3)
            po_d3 = self.po_UpConv3(po_d3)
            #po1_d3 = self.po_UpConv3(po1_d3)
            pr_d2 = self.pr_Up2(pr_d3)
            pr1_d2 = self.pr_Up2(pr1_d3)
            po_d2 = self.po_Up2(po_d3)
            #po1_d2 = self.po_Up2(po1_d3)
            # x2->x1
            pr_d2 = torch.cat((pr_x2, pr_d2), dim=1)
            pr1_d2 = torch.cat((pr1_x2, pr1_d2), dim=1)
            po_d2 = torch.cat((po_x2, po_d2), dim=1)
            #po1_d2 = torch.cat((po1_x2, po1_d2), dim=1)
            pr_d2 = self.pr_UpConv2(pr_d2)
            pr1_d2 = self.pr_UpConv2(pr1_d2)
            po_d2 = self.po_UpConv2(po_d2)
            #po1_d2 = self.po_UpConv2(po1_d2)
            pr_d1 = self.pr_Up1(pr_d2)
            pr1_d1 = self.pr_Up1(pr1_d2)
            po_d1 = self.pr_Up1(po_d2)
            #po1_d1 = self.pr_Up1(po1_d2)
            # cat
            pr_d1 = torch.cat((pr_x1, pr_d1), dim=1)
            pr1_d1 = torch.cat((pr1_x1, pr1_d1), dim=1)
            po_d1 = torch.cat((po_x1, po_d1), dim=1)
            #po1_d1 = torch.cat((po1_x1, po1_d1), dim=1)
            # x1->dis
            pr_u_dist, pr_s_dist, _, _, _, _ = self.compute_z_pr(pr_d1)
            # pr1_u_dist, pr1_s_dist, _, _, _, _ = self.compute_z_pr(pr1_d1)
            po_u_dist, po_s_dist, _, _, _, _ = self.compute_z_po(po_d1)
            #po1_u_dist, po1_s_dist, _, _, _, _ = self.compute_z_po(po1_d1)
            ...
            #po1_latent_u = po1_u_dist.rsample()
            #po1_latent_s = po1_s_dist.rsample()
            #po1_latent_u = torch.unsqueeze(po1_latent_u, -1)
            #po1_latent_u = torch.unsqueeze(po1_latent_u, -1)
            #po1_latent_s = torch.unsqueeze(po1_latent_s, -1)
            #po1_latent_s = torch.unsqueeze(po1_latent_s, -1)
            ...
            #po1_u = self.conv_u(po1_latent_u)
            #po1_s = self.conv_s(po1_latent_s)


            pr_d1 = self.insnorm(pr_d1) * torch.abs(po_s) + po_u
            #pr1_d1 = self.insnorm(pr1_d1) * torch.abs(po1_s) + po1_u
            # x1->out
            pr_d1 = self.pr_UpConv1(pr_d1)
           # print(pr_d1.size())
            pr_d1 = self.enhance(pr_d1)
            pr1_d1 = self.pr_UpConv1(pr1_d1)
            pr1_d1 = self.enhance(pr1_d1)
            pr_end=torch.cat((pr1_d1, pr_d1), dim=1)
            out = self.out_conv(pr_end)
            ...
            # pr1_latent_u = pr1_u_dist.rsample()
            # pr1_latent_s = pr1_s_dist.rsample()
            # pr1_latent_u = torch.unsqueeze(pr1_latent_u, -1)
            # pr1_latent_u = torch.unsqueeze(pr1_latent_u, -1)
            # pr1_latent_s = torch.unsqueeze(pr1_latent_s, -1)
            # pr1_latent_s = torch.unsqueeze(pr1_latent_s, -1)

            pr_u = self.conv_u(pr_latent_u)
            pr_s = self.conv_s(pr_latent_s)

            #pr1_u = self.conv_u(pr1_latent_u)
            #pr1_s = self.conv_s(pr1_latent_s)
            pr_d1 = self.insnorm(pr_d1) * torch.abs(pr_s) + pr_u
            #pr1_d1 = self.insnorm(pr1_d1) * torch.abs(pr1_s) + pr_u
            # x1->out
            pr_d1 = self.pr_UpConv1(pr_d1)
            #print(pr_d1.size())
            pr_d1 = self.enhance(pr_d1)
...
class mynet(nn.Module):
    ...
    def forward(self, Input, InputLab, label, training=True):
        self.Input = Input
        self.InputLab=InputLab
        self.label = label
        #self.labelLab=lableLab
        if training:
            self.out, self.pr_u, self.pr_s, self.po_u, self.po_s = self.decoder.forward(Input, InputLab, label)

    def sample(self, testing=False):
        if testing:
            self.out = self.decoder.forward(self.Input, self.InputLab, self.label, training=False)
            return self.out
    ...
    def kl_divergence(self, analytic=True):
        if analytic:
            ...
            #kl1_div_u = torch.mean(kl.kl_divergence(self.po1_u, self.pr1_u))
            #kl1_div_s = torch.mean(kl.kl_divergence(self.po1_s, self.pr1_s))
